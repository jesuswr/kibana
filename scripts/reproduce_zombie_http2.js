...
/**
 * reproduce_zombie_http2.js
 *
 * Reproduces the Node.js HTTP/2 zombie session bug (nodejs/node#61304)
 * on macOS using the pf (packet filter) firewall to create a network
 * black hole.
 *
 * REQUIRES: sudo (pf firewall manipulation needs root)
 * PLATFORM: macOS only (uses pfctl)
 *
 * Usage:
 *   sudo node scripts/reproduce_zombie_http2.js                # Demonstrate zombie state only
 *   sudo node scripts/reproduce_zombie_http2.js --crash         # Also trigger the assertion crash
 *   sudo node scripts/reproduce_zombie_http2.js --port 9443     # Use a custom port
 *   sudo node scripts/reproduce_zombie_http2.js --help
 *
 * What it does:
 ...
 *   3. Activates a pf firewall rule to silently drop all packets to the server port
 ...
 *   7. Cleans up the firewall rule (even on crash, via signal handlers)
 *
 * Cleanup:
 *   The script registers signal handlers and an exit hook to remove the firewall rule.
...
const PF_CONF_TMP = '/tmp/zombie_http2_pf.conf';

// Use Kibana's existing test certificates
const CERTS_DIR = path.resolve(__dirname, '../src/platform/packages/shared/kbn-dev-utils/certs');
...
/**
 * Runs a shell command, swallowing stderr (pf likes to print to stderr).
...
//
// macOS's default /etc/pf.conf only evaluates anchors under "com.apple/*".
// A custom top-level anchor would never be evaluated. Instead, we temporarily
// replace the main ruleset with one that:
//   1. Blocks our test port on lo0 (the black hole)
//   2. Preserves Apple's anchor points so system services keep working
//   3. Passes everything else
// On cleanup we restore /etc/pf.conf.
// ---------------------------------------------------------------------------

function pfCleanup() {
  sh('pfctl -f /etc/pf.conf 2>/dev/null');
  try { fs.unlinkSync(PF_CONF_TMP); } catch { }
}

function pfBlockPort(port) {
  // "block drop on lo0" = silently discard loopback packets (no RST, no ICMP)
  // "quick" = stop evaluating rules after this match
  // We block BOTH directions to fully simulate a network partition:
  //   - client -> server: requests disappear
  //   - server -> client: responses/ACKs disappear
  // Minimal ruleset: block our port on loopback, pass everything else.
  // Apple anchors are omitted — they're not needed for a 30-second test.
  // /etc/pf.conf is restored on cleanup.
  const conf = [
    `block drop quick on lo0 proto tcp from any to 127.0.0.1 port ${port}`,
    `block drop quick on lo0 proto tcp from 127.0.0.1 port ${port} to any`,
    'pass all',
  ].join('\n') + '\n';

  fs.writeFileSync(PF_CONF_TMP, conf);

  // Load the config. pfctl writes status info to stderr even on success,
  // so capture stderr for diagnostics. If it fails, show what went wrong.
  try {
    execSync(`pfctl -f ${PF_CONF_TMP}`, { stdio: ['ignore', 'pipe', 'pipe'] });
  } catch (err) {
    const stderr = err.stderr ? err.stderr.toString().trim() : '(no output)';
    log('PF', `pfctl -f stderr: ${stderr}`);
    // pfctl exits non-zero even for "rules loaded" — only rethrow on real parse errors
    if (stderr.includes('syntax error') || stderr.includes('unknown')) {
      throw new Error(`pfctl -f failed: ${stderr}`);
    }
  }
  sh('pfctl -e 2>/dev/null');
}
...
function preflight() {
  if (process.platform !== 'darwin') {
    console.error('This script uses macOS pf firewall. For Linux, use iptables instead.');
    process.exit(1);
  }

  if (process.getuid() !== 0) {
    console.error('This script requires root for firewall manipulation.');
    console.error('Run with: sudo node scripts/reproduce_zombie_http2.js');
    process.exit(1);
  }
  ...
  // Clean up any leftover rules from a previous crashed run
  pfCleanup();
...
// ---------------------------------------------------------------------------
// Cleanup on exit (critical — leftover pf rules break networking)
// ---------------------------------------------------------------------------
...
function cleanup() {
  ...
  log('CLEANUP', 'Removing firewall rules...');
  pfCleanup();
...
async function main() {
  ...
  console.log(' HTTP/2 Zombie Session Reproduction (macOS)');
  ...
  log('PHASE 3', 'Activating firewall black hole...');
  log('PHASE 3', `  Rule: block drop all TCP to/from 127.0.0.1:${PORT}`);
  ...
  for (let i = 1; i <= MONITOR_SECONDS; i++) {
    ...
    // The session is a zombie if:
    //   - It reports as healthy (not closed, not destroyed)
    //   - No close/error events have fired
    //   - Requests sent into the black hole got no responses
    // Note: outboundQueueSize may be 0 — small requests fit in the kernel's
    // TCP send buffer, so they leave HTTP/2's queue immediately. The data is
    // stuck in the kernel, not in Node.js userspace.
    const isZombie =
      !session.closed &&
      !session.destroyed &&
      events.length === 0 &&
    ...
    console.log(`  │ [${elapsed}s] ${statusIcon} | closed=${session.closed} destroyed=${session.destroyed} responses=0/${pendingRequests.length} events=${events.length === 0 ? 'none' : events.join(',')}`);
  ...
  if (responsesReceived === 0 && events.length === 0) {
    ...
    log('RESULT', '  Check that pf rules loaded correctly.');
  }
  ...
  if (TRIGGER_CRASH) {
    ...
    // If we get here, the crash didn't happen (unlikely with the pf approach)
    await sleep(2000);
